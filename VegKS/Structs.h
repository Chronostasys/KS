...
struct TertiaryList
{
	...
	TertiaryNode* Where(std::function<int(TertiaryNode*)> lambda) {
	...
	TertiaryNode* While(std::function<int(TertiaryNode*)> lambda) {
...
struct SecondaryList
{
	...
	SecondaryNode* Where(std::function<int(SecondaryNode*)> lambda) {
	...
	SecondaryNode* While(std::function<int(SecondaryNode*)> lambda) {
...
struct PrimaryList
{
	...
	PrimaryNode* Where(std::function<int(PrimaryNode*)> lambda) {
	...
	PrimaryNode* While(std::function<int(PrimaryNode*)> lambda) {
		PrimaryNode* current = head;
		while (true)
		{
			current = current->nextVegClass;
